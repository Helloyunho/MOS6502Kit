...
public struct MOS6502 {
    ...
    /**
     ...
     > Note: When implied mode is selected, there's no need for this function
       to work and therefore it returns 0.
     */
    ...
    private func runAndWait(for cycle: Int, action: () async -> Void) async {
    ...
    /**
     Repeatedly executes the current op code in memory.
    ...
    public mutating func run() async {
        while !self[.B] {
            let op = memory[PC]
            if let cycle = MOS6502.cycleLists[op] {
